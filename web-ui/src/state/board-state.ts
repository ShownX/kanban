...
export function normalizeBoardData(rawBoard: unknown): BoardData | null {
	...
	return runtimeTaskState.updateTaskDependencies({
		...
		roadmap: normalizeRoadmap((rawBoard as { roadmap?: unknown }).roadmap),
		roadmapAnnotations: normalizeAnnotations((rawBoard as { roadmapAnnotations?: unknown }).roadmapAnnotations),
	});
}

function normalizeRoadmap(raw: unknown): BoardData["roadmap"] {
	if (!Array.isArray(raw)) return [];
	return raw.filter(
		(item): item is BoardData["roadmap"][number] =>
			item !== null &&
			typeof item === "object" &&
...
function normalizeAnnotations(raw: unknown): BoardData["roadmapAnnotations"] {
	if (!Array.isArray(raw)) return [];
	return raw.filter(
		(item): item is BoardData["roadmapAnnotations"][number] =>
			item !== null &&
			typeof item === "object" &&
			typeof (item as Record<string, unknown>).id === "string" &&
